...
- [ ] **T007** LLM proxy service: Gemini 2.5 Flash primary with Haiku fallback, prompt builder that injects user memory context, structured JSON output, per-user budget guards + logging.
  - **Status (2026-07-14 Tester note)**: Agents spawned (impl 019f5dad-8928..., rev 019f5dad-a0aa..., test ...); monitoring shows no code delivered yet (no llm_service.py; lead_service has explicit "LLM in T007" stubs). Foundation (T008 orchestration + CrmClient + BdLeadResult shape) verified green + ready. Mocks for Gemini+Haiku/memory/JSON/budgets/logging + full flow pass. See REVIEW_FOR_T007.md + BUILD_COORDINATION.md append. Awaiting impl delivery + updates per rule. Builds re-ran clean.
...
- [x] **T008** Lead orchestration service that reuses/refactors logic from `src/tool.ts` (contact → deal → 3 tasks) using the CrmClient.
