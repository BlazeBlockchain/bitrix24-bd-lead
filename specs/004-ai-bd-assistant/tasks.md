...
- [ ] **T009** Postgres models + migrations for User, Lead, CrmConnection, MemoryProfile, OutreachHistory (see data-model.md).
  - **Review prep (2026-07-14)**: REVIEW_FOR_T009.md created (pre-delivery). Full protocol reads (BUILD_COORDINATION, plan, data-model, ARCH, backend stubs). Checklist + expected patches documented. Current stubs + alembic + vault comments + db layer green for T009. No impl code delivered yet (no new models/lead.py etc). Pre-verifs (syntax, TS clean, compose, Crm mocks both prov) PASS. Awaiting impl for exact schema match + mig + T010 prep. See REVIEW_FOR_T009.md + BUILD_COORDINATION append. All agents: update mds on delivery.
