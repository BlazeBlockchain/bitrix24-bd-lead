...
- [x] **T007** LLM proxy service: Gemini 2.5 Flash primary with Haiku fallback, prompt builder that injects user memory context, structured JSON output, per-user budget guards + logging.
  - Impl in `backend/app/services/llm_service.py`; integrated pre-CRM in T008 lead_service + added /api/leads/enrich (protected).
  - **Tester/Committer (2026-07-14)**: FULL VERIF GREEN. Direct mock tests: Gemini/Haiku paths (via mock), memory ctx, structured {company_snapshot, personalized_opener, follow_ups[3x{title,desc,due_in_days,rationale}]} + model/memory_note. Budget exceed -> mock + note. Integration: create_lead_with_followups for bitrix24+hubspot (via mock CrmClient factory): exact 1 createContact + 1 createDeal(linked contactId) + 3 createTask(dealId, due +4/9/14), enriched_preview present in result. Return shape matches BdLeadResult + additive. Builds: npm run build + tsc clean; py_compile green. No breakage: Crm adapters untouched, src/tool.ts + execute ref preserved, web preview shape compat. See appended log in BUILD_COORDINATION.md + updated REVIEW_FOR_T007.md. Ready for commit.
...
- [x] **T009** Postgres models + migrations for User, Lead, CrmConnection, MemoryProfile, OutreachHistory, UsageLedger (see data-model.md).
  - **Reviewer (2026-07-14)**: STRONG PASS with patches applied for exact match. Models now match data-model columns/relations/indexes exactly (users, crm_connections, leads, user_memory_profiles, outreach_history, usage_ledger). New 002 migration + pgvector dep + full relationships + current_user integration prep (via id in vault/lead_service). No breakage to T005 get_current_user, T006 vault (resolve comments + CrmConnection), T008 lead_service (current_user passed, no db dep change), T007 llm. Prepares T010 (models ready for queries in enrich/push/history). Updated __init__, database, alembic/env, reqs. Verifs: py_compile green, syntax ok. See full REVIEW_FOR_T009.md + BUILD_COORDINATION append for findings/patches/shared. All .md updated. Ready for T010 + tester commit.
